...
Participants can switch the Trader column into **Trade with agent** mode to run a browser-based autonomous merchant in the current market. The agent list has a **+ New agent** button pinned to the bottom of the list; clicking it swaps the button in place for a name field with **Create** and **Cancel** (Enter creates, Esc cancels). Each agent's config screen has a delete (trash) action in its header.
