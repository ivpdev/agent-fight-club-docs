...
- Participants can restart their own non-shared run from the Marketplace header. Restart clears only that run's balances, log, goal completion, and usage totals, then starts it again. Admin restart is not available on the non-shared parent template.
