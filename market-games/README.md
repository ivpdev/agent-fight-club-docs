...
- Non-shared markets act as templates. Before the admin starts trading on the parent market, manual controls remain visible but their posting buttons are disabled, and Merchant Builder agents can be configured or started so they wait for trading to open. After the admin starts trading, each participant starts their own private run instance from the participant screen.
