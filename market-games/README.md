...
- A single player participant run is `trade` while active and `closed` once finished. It closes when its goal is reached or the participant closes it. The market itself stays `open` for other participants to start their own runs.
...
- Participants can start, restart, or close their own single player run from the Marketplace header. Restart closes the current run (which remains in history) and starts a fresh one with reset balances, log, goal completion, and usage totals. Admins can reset a single player market, which closes any active runs (kept in history) and reopens it for new runs.
...
Open `/market/ui/markets` to see the markets available to you. Use **Open trade view** to open the trading view, or **Admin** to manage a market you created.
